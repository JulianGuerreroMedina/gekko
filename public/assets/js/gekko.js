...
function imagenes() {
  ...
  fetch(`/media/${imagen}`) // Verificar si existe la imagen
    .then((response) => {
      if (response.ok) {
          // la imagen si existe
...
function videos() {
  ...
  fetch(`/media/${video}`) // Verificar si el video existe
    .then((response) => {
      if (response.ok) {
          // Video existe
          let render = `<video src="/media/${video}" id="myVideo" autoplay="autoplay" muted="">video not supported</video>`;
          document.querySelector("#contenedor").insertAdjacentHTML("beforeend", render);
